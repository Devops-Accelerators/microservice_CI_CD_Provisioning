properties([parameters([[$class: 'GlobalVariableStringParameterDefinition', defaultValue: '', description: 'name of the microservice/repo name', name: 'MicroserviceName'], [$class: 'GlobalVariableStringParameterDefinition', defaultValue: '', description: 'HTTPs Url of the github repository', name: 'apiRepoURL'], [$class: 'GlobalVariableStringParameterDefinition', defaultValue: '', description: 'port for kubernates', name: 'port'], credentials(credentialType: 'com.cloudbees.plugins.credentials.impl.UsernamePasswordCredentialsImpl', defaultValue: '', description: 'enter git credential for private repository ', name: 'gitCred', required: false), password(defaultValue: '', description: 'enter github password only', name: 'gitpassword')])])
...
sonar.test.exclusions=src/test/java/com/mindtree/BasicApp"""
					...
					#git push -f origin master 
					 git push https://${gituserName}:${gitpassword}@${gituserName}/${repoName.trim()}.git --all
					cd ..
					rm -rf ${repoName.trim()}"""	
			}
			}
